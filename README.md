...
STRM 直链。除视频流外的控制请求均按 demo 的方式透明代理到 Emby：请求方法、查询参数、
认证 Header 和原始请求体保持不变，`Sessions/Playing`、`Progress`、`Stopped` 由 Emby
原生记录；WebSocket 控制通道也会双向透传。管理页会显示请求量、302 次数、缓存命中率、
反代数量、错误数量和最近跳转记录。
