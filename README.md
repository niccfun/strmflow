...
5. OpenList 的 Strm 来源路径或网盘挂载路径发生变化后，先修正 OpenList 存储配置，再在媒体卡片中点击“重新同步”。系统会重新扫描源目录，并用最新生成的内容覆盖目标目录中已有的 STRM；网盘原视频不会被删除。
...
分享接口返回链接失效状态后，系统会暂停该媒体的自动追更，避免持续请求无效链接。“仅百度定时检查”模式会按通知配置发送一次失效提醒；“Telegram 实时 + 百度兜底”模式不会发送失效提醒，而是静默等待后续 Telegram 消息提供新链接并自动恢复追更。用户也可以在媒体详情中手动更新分享链接恢复检查。
...
- **Telegram 实时 + 百度兜底**：Telegram 消息负责实时发现更新，百度检查周期作为漏消息时的低频兜底；当前链接失效后静默暂停，收到后续 Telegram 新链接时自动恢复，不发送链接失效通知。
...
收到消息后，系统按消息块提取媒体标题、季号、年份、更新集数和百度链接，只匹配状态为“更新中”的现有媒体。匹配成功时会更新已失效或已变化的百度链接、恢复该媒体追更，并直接加入一次百度分享即时检查，不等待兜底周期；实际转存仍由现有 `bdpan` 流程完成。即使消息中的链接没有变化，也会检查同一分享中的新集和本地缺集。频道消息编辑也会被处理，同一消息内容不会重复消费。
...
- 剧集更新通知；
- 分享链接失效通知（仅百度定时检查模式；混合模式静默等待 Telegram 更新链接）。
...
日志中的 `linkChanged:false` 仅表示 Telegram 消息中的分享链接与媒体当前链接相同，不代表跳过更新检查。后续应看到“Telegram 已触发百度网盘即时检查”；如果百度网盘自动追更开关未启用，日志会明确记录未执行原因。
