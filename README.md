...
3. 后续检查按字符串保存官方返回的 `fs_id`，同时核对分享剧集与已保存 STRM 清单；新增、
   漏存或出现更高质量版本时，按原相对目录分组并提交 `transfer select`。同集多文件依次
   优先分辨率、HDR/Dolby Vision、帧率、片源、编码和文件大小，只保留最优版本进入同步。
   同一文件提交后立即记入状态，避免异步任务尚未完成时重复提交。
